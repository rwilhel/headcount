...
class EconomicProfileTest < Minitest::Test
  ...
  def test_it_can_find_children_in_poverty_in_given_year
    ...
    assert_raises(UnknownDataError){economic_profile.children_in_poverty_in_year(1)}
  end

  def test_it_can_return_free_or_reduced_price_lunch_percentage_in_year
    epr = EconomicProfileRepository.new
    epr.load_data({
                    :economic_profile => {
                      :median_household_income => "./data/Median household income.csv",
    ...
    assert_equal 0.12539, economic_profile.free_or_reduced_price_lunch_percentage_in_year(2012)

    assert_raises(UnknownDataError){economic_profile.free_or_reduced_price_lunch_percentage_in_year(1)}
  end

  def test_it_can_return_total_on_free_or_reduced_lunch_in_year
    epr = EconomicProfileRepository.new
    epr.load_data({
                    :economic_profile => {
                      :median_household_income => "./data/Median household income.csv",
    ...
    assert_equal 3006, economic_profile.free_or_reduced_price_lunch_number_in_year(2012)

    assert_raises(UnknownDataError){economic_profile.free_or_reduced_price_lunch_number_in_year(1)}
  end

  def test_it_can_return_title_i_percentage_as_float
    epr = EconomicProfileRepository.new
    epr.load_data({
                    :economic_profile => {
                      :median_household_income => "./data/Median household income.csv",
