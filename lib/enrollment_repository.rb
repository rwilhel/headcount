...
class EnrollmentRepository
  ...
  def load_data(data)
    ...
    if data[:enrollment][:kindergarten]
      kindergarten_contents = CSV.open data[:enrollment][:kindergarten], headers: true,
    ...
    if data[:enrollment][:high_school_graduation]
      high_school_contents = CSV.open data[:enrollment][:high_school_graduation], headers: true,
      header_converters: :symbol
  ...
  enrollment_arguments = {:name => name, :kindergarten_participation =>
    enrollment_year_and_data, :high_school_graduation => graduation_year_and_data}
